import { FileAudio, FileJson, Filter, MoreHorizontal, Music2, Pause, Play, Search, Trash2, Upload, Video } from 'lucide-react'
...
const taskTypeLabels: Record<string, string> = { separated: '分离任务', analysis: '分析任务', generated: '生成任务' }
...
export function LibraryPage() {
  ...
  async function upload(file: File) {
    ...
    try {
      ...
      await audioAssetsApi.uploadAsset({ file, projectId: selectedProject.project_id, subtype: 'full', duration })
      await reload()
  ...
  async function remove(asset: Asset) {
    if (!window.confirm(`确认删除“${assetName(asset)}”？\n数据库记录和 MinIO 文件会一并删除。`)) return
  ...
  return (
    <div className="library-page">
      <PageHeader
        ...
        title="资产库"
        description="所有音频、分轨、分析文件和生成结果都在这里。"
          ...
          <tbody>
            {filteredGroups.map((group, index) => (
              <Fragment key={group.root?.asset_id || `orphan-${index}`}>
                ...
                {groupedByTask(group.children).map((taskGroup) => (
...
function AssetRow({
  ...
  return (
    <tr className={`${isChild ? 'asset-child-row' : 'asset-root-row'} ${isContextOnly ? 'is-context-only' : ''}`}>
      ...
      <td><span className={`asset-type type-${asset.type}`}>{typeLabels[asset.type] || asset.type}</span></td>
      ...
      <td><button className="table-icon" title="更多信息"><MoreHorizontal size={17} /></button><button className="table-icon delete" disabled={disabledDelete || deletingId === asset.asset_id} onClick={() => void onRemove(asset)} title={disabledDelete ? '请先删除下方衍生资产' : '删除资产'}><Trash2 size={16} /></button></td>
    </tr>
  )
}

function buildAssetGroups(assets: Asset[], tab: (typeof tabs)[number], query: string): AssetGroup[] {
  const rawAssets = assets.filter((asset) => asset.type === 'raw').sort(sortByCreatedDesc)
  const derivedAssets = assets.filter((asset) => asset.type !== 'raw').sort(sortByCreatedDesc)
  ...
  const groups: AssetGroup[] = rawAssets.map((root) => ({
    root,
    children: childrenByParent.get(root.asset_id) || [],
  }))
  if (orphans.length) groups.push({ root: null, children: orphans })
...
function groupedByTask(assets: Asset[]) {
  ...
  return Array.from(buckets.entries()).map(([taskId, records]) => ({ taskId, assets: records.sort(sortBySubtype) }))
}
